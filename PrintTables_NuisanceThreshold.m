...
[~,~,data] = xlsread([baseDir '/Thresholds/Nuisance_Thresholds.xlsx']);
for idx = 2:size(data,1)
    if data{idx,4} ~= 0
        spmfile = {fullfile(baseDir,'Second_Level_nuisance',data{idx,1},data{idx,2},'SPM.mat')};
        cd(fullfile(baseDir,'Second_Level_nuisance',data{idx,1},data{idx,2}));
        ...
        command = sprintf('mv %s/spm_2018Jul26_001.csv %s/%s/%s%s_table.csv',pwd,outputDir,data{idx,1},data{idx,2},pval(3:end));
        system(command)
    end
end
