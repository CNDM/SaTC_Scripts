...
if ~exist(fullfile(mriSubdir,'Benefit','Benefit_Model'))
    mkdir(fullfile(mriSubdir,'Benefit','Benefit_Model'));
end
modelDir = fullfile(mriSubdir,'Benefit','Benefit_Model');
...
%   Read in regressors
    cd(BehaviorSubdir)
    Regressors = csvread(fullfile(BehaviorSubdir, sprintf('BenefitRegressor.%s',sNum)));
...
%%
matlabbatch{1}.spm.stats.fmri_spec.sess.cond.name = 'Benefit';
...
%%
matlabbatch{1}.spm.stats.fmri_spec.sess.cond.pmod.name = 'Rating';
matlabbatch{1}.spm.stats.fmri_spec.sess.cond.pmod.param = Regressors(:,2);
