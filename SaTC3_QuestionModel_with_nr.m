function SaTC3_QuestionModel_with_nr(s)
...
if ~exist(fullfile(mriSubdir,'Question','Question_Model'))
    mkdir(fullfile(mriSubdir,'Question','Question_Model'));
end
modelDir = fullfile(mriSubdir,'Question','Question_Model');
...
%   Read in regressors
    cd(BehaviorSubdir)
    rawRegressors = csvread(fullfile(BehaviorSubdir, sprintf('QuestionRegressor.%s',sNum)));
...
%%
matlabbatch{1}.spm.stats.fmri_spec.sess.cond(1).name = 'Question';
...
%%
matlabbatch{1}.spm.stats.fmri_spec.sess.cond(1).pmod.name = 'Intrusiveness';
matlabbatch{1}.spm.stats.fmri_spec.sess.cond(1).pmod.param = Regressors(:,2);
