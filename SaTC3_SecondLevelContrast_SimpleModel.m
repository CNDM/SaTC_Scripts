function SaTC3_SecondLevelContrast_SimpleModel(s,task)
...
if length(s) < 10
    outputdir=[sprintf('%s%sModelN0%d/',datadir,task,length(s))];
else
    outputdir=[sprintf('%s%sModelN%d/',datadir,task,length(s))];
end

%Exception to add the '1' for the Decision folder
if strcmp(task,'Decision') 
    modeldir = sprintf('%s%s_3D/%s1/%s_Model/',datadir, sNum,task,task);
else
    modeldir = sprintf('%s%s_3D/%s/%s_Model/',datadir, sNum,task,task);
end
...
for c=1:cons
    ...
    for sub=1:length(s)
        if c<10
            if strcmp(task,'Decision')
                modeldir = sprintf('%s%s_3D/%s1/%s_Model/',datadir, num2str(s(sub)),task,task);
            else
                modeldir = sprintf('%s%s_3D/%s/%s_Model/',datadir, num2str(s(sub)),task,task);
            end
