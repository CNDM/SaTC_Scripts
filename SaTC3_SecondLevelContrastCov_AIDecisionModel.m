function SaTC3_SecondLevelContrastCov_AIDecisionModel(subj,subs,covfile,directory)
% % List of open inputs
...
outputdir=[fullfile(directory.datadir, 'AIDecisionModelN24/')];
conspm=[directory.datadir '/' num2str(subs(1)) '_3D/Decision1/AI_Decision_Model/SPM.mat'];
cmd = ['load ' [conspm]];
eval(cmd);
...
for c=1:cons
    ...
    for s=1:length(subj)
        if c<10
            scans(s)={[sprintf('%s/%s_3D/Decision1/AI_Decision_Model/con_000%s.nii,1',directory.datadir, num2str(subs(s)),num2str(c))]};
        else
            scans(s)={[sprintf('%s/%s_3D/Decision1/AI_Decision_Model/con_00%s.nii,1',directory.datadir, num2str(subs(s)),num2str(c))]};
        end
    end %subj loop
