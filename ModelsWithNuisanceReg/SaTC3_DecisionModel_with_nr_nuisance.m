function SaTC3_DecisionModel_with_nr_nuisance(s)
...
if ~exist(fullfile(mriSubdir,'Decision1','Decision_Model_nuisance'))
    mkdir(fullfile(mriSubdir,'Decision1','Decision_Model_nuisance'));
end
modelDir = fullfile(mriSubdir,'Decision1','Decision_Model_nuisance');
    DecisionSession = [];
...
%   Read in regressors
    cd(BehaviorSubdir)
    rawRegressors = csvread(fullfile(BehaviorSubdir, sprintf('DecisionRegressor.%s',sNum)));
...
save(fullfile(modelDir,['Decision' date '.mat']),'matlabbatch');
cd(modelDir)
spm('defaults', 'FMRI');
