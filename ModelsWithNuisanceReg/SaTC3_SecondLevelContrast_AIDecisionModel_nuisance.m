function SaTC3_SecondLevelContrast_AIDecisionModel_nuisance(s)
...
if length(s) < 10
    outputdir=[sprintf('%sAIDecisionModelN0%d_withRes_nuisance/',datadir,length(s))];
else
    outputdir=[sprintf('%sAIDecisionModelN%d_withRes_nuisance/',datadir,length(s))];
end

%   Name model directory
modeldir = sprintf('%s%s_3D/Decision1/AI_Decision_Model_nuisance/',datadir, sNum);
...
for c=1:cons
    ...
    for sub=1:length(s)
        modeldir = sprintf('%s%s_3D/Decision1/AI_Decision_Model_nuisance/',datadir, num2str(s(sub)));
        if c<10
