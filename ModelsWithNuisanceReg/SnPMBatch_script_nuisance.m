function SnPMBatch_script_nuisance(s,model,threshold)
%-----------------------------------------------------------------------
...
sNum = num2str(s(1));
output_header = sprintf('/Users/CNDM/Dropbox/SaTCPrivacyGrant/SaTC3/fMRIAnalyses/SnPM/SnPM_%s_nuisance/%s/',model,threshold)
%-----------------------
%   Dropbox Directory
...
if strcmp(model,'Decision') | strcmp(model,'AI_Decision')
    modeldir_name = 'Decision1';
else
    modeldir_name = model;
end

modeldir = sprintf('%s%s_3D/%s/%s_Model_nuisance/',datadir, sNum, modeldir_name, model);
...
for c=1:cons
    ...
    for sub=1:length(s)
        modeldir = sprintf('%s%s_3D/%s/%s_Model_nuisance/',datadir, num2str(s(sub)),modeldir_name,model);
        if c<10
...
matlabbatch{3}.spm.tools.snpm.inference.Thr.Clus.ClusSize.CFth = str2num(threshold);
