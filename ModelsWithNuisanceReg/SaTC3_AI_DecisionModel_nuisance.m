function SaTC3_AI_DecisionModel_nuisance(s)
...
if ~exist(fullfile(mriSubdir,'Decision1','AI_Decision_Model_nuisance'))
    mkdir(fullfile(mriSubdir,'Decision1','AI_Decision_Model_nuisance'));
end
modelDir = fullfile(mriSubdir,'Decision1','AI_Decision_Model_nuisance');
    DecisionSession = [];
...
%   Get rid of missing responses
    ...
    DecRegressor = Regressors(Regressors(:,2)~=0,:);
    
%   Build Junk Regressor
    MissingResponses = sum([Questions', Benefits', Regressors(:,2)]')==0;
    
    modulator = DecRegressor(:,3) - mean(DecRegressor(:,3));
...
%%  Decision pmod
matlabbatch{1}.spm.stats.fmri_spec.sess.cond(1).pmod(1).name = 'Willingness';
matlabbatch{1}.spm.stats.fmri_spec.sess.cond(1).pmod(1).param = DecRegressor(:,2);
