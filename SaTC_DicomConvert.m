...
mainDir = '/Users/CNDM/Documents/SaTC3_withPractice/fMRIAnalyses/fMRI_Data/Raw_Scans'; %CNDM
%Laptop Setup. 
...
outputdir = '/Users/CNDM/Documents/SaTC3_withPractice/fMRIAnalyses/fMRI_Data/ReconScans'; %Where should images be output? Setup for Crystal's laptop
...
try
    ...
    for i = 1:size(folders,1)
        ...
        if strfind(folders{i,1},'satc_d1')
            try
                inputs.nonmoco_index(i,1) = str2num(folders{i,1}(1:2));
            catch
                inputs.nonmoco_index(i,1) = str2num(folders{i,1}(1));
        ...
        if strfind(folders{i,1},'satc_d2')
            try
                inputs.nonmoco_index(i,1) = str2num(folders{i,1}(1:2));
            catch
                inputs.nonmoco_index(i,1) = str2num(folders{i,1}(1));
    ...
    if ismember(mod(str2num(subjectID(1:3)),4),[0,1])
        runNames = {'Question','Benefit','Decision1','Decision2'};
    else
        runNames = {'Benefit','Question','Decision1','Decision2'};
    end
    ...
    images = dir(fullfile(outputdir,subjectID,'Benefit','Non MoCo','*001.nii')); %change accordingly based on file name
    volinfo = spm_vol(fullfile(outputdir,subjectID,'Benefit','Non MoCo',char(images(1,1).name)));
