...
subs = [302:307,311:322, 324:326, 328, 329];
    ...
    for c = contrasts % Loop through the 3 contrast folders
        switch c{:};
            case 'PosIntr' %Change to appropriate directory, appropriate VOI names
                cd(sprintf('%s%s_3D/Decision1/AI_Decision_Model_nuisance/Positive_Intrusiveness_PPI',datadir,s));
                voi = {'LAIns','RAIns','LPPC','DMPFC','PCC','LVis','RPFC','LPFC'};
            case 'PosWill'
                cd(sprintf('%s%s_3D/Decision1/AI_Decision_Model_nuisance/Positive_Willingness_PPI',datadir,s));
                voi = {'LPPC','Rcere','Lamyg','LDLPFC'};
            case 'NegWill'
                cd(sprintf('%s%s_3D/Decision1/AI_Decision_Model_nuisance/Negative_Willingness_PPI',datadir,s));
                voi = {'RPPC','LVis'};
        end
        contrDir = pwd;
